...
main = run [consoleReporter] do
  ...
  describe "set tests" do
      ...
      describe "should stack with lenses" do
        ...
        it "stacks with traversals" do
          (pprint $ (traversal >>> lens) setbase) `shouldEqual` "export function set<T1, A2>(t1: Traversal<T1>, l2: Lens<T1, A2>): (v: A2) => <S extends Collection<T1>>(s: S) => S"
          (pprint $ (lens >>> traversal) setbase) `shouldEqual` "export function set<S1, A1 extends Collection<T2>, T2>(l1: Lens<S1, A1>, t2: Traversal<T2>): (v: T2) => (s: S1) => S1"
      ...
      describe "spot check of complex combos" do
        ...
        it "LLPTP" do
          (pprint $ (lens >>> lens >>> path >>> traversal >>> path) setbase) `shouldEqual` "export function set<S1, A1, A2 extends HasKey<K3, Collection<T4 & HasKey<K5>>>, K3 extends string, T4, K5 extends string>(l1: Lens<S1, A1>, l2: Lens<A1, A2>, k3: K3, t4: Traversal<T4>, k5: K5): (v: Unpack<A4[K3]>[K5]) => (s: S1) => S1"
        it "LLPTL" do
  ...
  --       (pprint $ traversal modbase) `shouldEqual` "export function mod<T1>(t1: Traversal<T1>): (f: (v: T1) => T1) => <S extends Collection<T1>>(s: S) => S"
  ...
  --     describe "should stack with traversals" do
  --       it "should handle paths" do
  --         (pprint $ (path >>> traversal) modbase) `shouldEqual` "export function mod<K1 extends string, T2>(k1: K1, t2: Traversal<T2>): (f: (v: T2) => T2) => <S extends HasKey<K1, Collection<T2>>>(s: S) => S"
  --         (pprint $ (traversal >>> path) modbase) `shouldEqual` "export function mod<T1, K2 extends string>(t1: Traversal<T1>, k2: K2): (f: (v: T1[K2]) => T1[K2]) => <S extends Collection<T1 & HasKey<K2>>>(s: S) => S"          
            
  --       it "should handle index" do
  --         (pprint $ (traversal >>> idx) modbase) `shouldEqual` "export function mod<T1>(t1: Traversal<T1>, i2: number): (f: (v: Index<T1>) => Index<T1>) => <S extends Collection<T1>>(s: S) => S"
  --         (pprint $ (idx >>> traversal) modbase) `shouldEqual` "export function mod<T2>(i1: number, t2: Traversal<T2>): (f: (v: T2) => T2) => <S extends Indexable<Collection<T2>>>(s: S) => S"
            
  --       it "should handle traversal on traversal actions" do
  --         (pprint $ (traversal >>> traversal) modbase) `shouldEqual` "export function mod<T1, T2>(t1: Traversal<T1>, t2: Traversal<T2>): (f: (v: T2) => T2) => <S extends Collection<T1 & Collection<T2>>>(s: S) => S"

  --       it "should only apply constraints to last traversal" do
  --         (pprint $ (traversal >>> traversal >>> path) modbase) `shouldEqual` "export function mod<T1, T2, K3 extends string>(t1: Traversal<T1>, t2: Traversal<T2>, k3: K3): (f: (v: T2[K3]) => T2[K3]) => <S extends Collection<T1 & Collection<T2 & HasKey<K3>>>>(s: S) => S"
  ...
  --       it "stacks with traversals" do
  --         (pprint $ (traversal >>> lens) modbase) `shouldEqual` "export function mod<T1, A2>(t1: Traversal<T1>, l2: Lens<T1, A2>): (f: (v: A2) => A2) => <S extends Collection<T1>>(s: S) => S"
  --         (pprint $ (lens >>> traversal) modbase) `shouldEqual` "export function mod<S1, A1 extends Collection<T2>, T2>(l1: Lens<S1, A1>, t2: Traversal<T2>): (f: (v: T2) => T2) => (s: S1) => S1"

  --     describe "spot check of complex combos" do
  --       it "PTP" do 
  --         (pprint $ (path >>> traversal >>> path) modbase) `shouldEqual` "export function mod<K1 extends string, T2, K3 extends string>(k1: K1, t2: Traversal<T2>, k3: K3): (f: (v: T2[K3]) => T2[K3]) => <S extends HasKey<K1, Collection<T2 & HasKey<K3>>>>(s: S) => S"
  --       it "TTPPI" do
  --         (pprint $ (traversal >>> traversal >>> path >>> path >>> idx) modbase) `shouldEqual` "export function mod<T1, T2, K3 extends string, K4 extends string>(t1: Traversal<T1>, t2: Traversal<T2>, k3: K3, k4: K4, i5: number): (f: (v: Index<T2[K3][K4]>) => Index<T2[K3][K4]>) => <S extends Collection<T1 & Collection<T2 & HasKey<K3, HasKey<K4, Indexable>>>>>(s: S) => S"
  --       it "ITKK" do 
  --         (pprint $ (idx >>> traversal >>> path >>> path) modbase) `shouldEqual` "export function mod<T2, K3 extends string, K4 extends string>(i1: number, t2: Traversal<T2>, k3: K3, k4: K4): (f: (v: T2[K3][K4]) => T2[K3][K4]) => <S extends Indexable<Collection<T2 & HasKey<K3, HasKey<K4>>>>>(s: S) => S"
  --       it "TIKK" do
  --         (pprint $ (traversal >>> idx >>> path >>> path) modbase) `shouldEqual` "export function mod<T1, K3 extends string, K4 extends string>(t1: Traversal<T1>, i2: number, k3: K3, k4: K4): (f: (v: Index<T1>[K3][K4]) => Index<T1>[K3][K4]) => <S extends Collection<T1 & Indexable<HasKey<K3, HasKey<K4>>>>>(s: S) => S"
  --       it "TPL" do
  --         (pprint $ (traversal >>> path >>> lens) modbase) `shouldEqual` "export function mod<T1, K2 extends string, A3>(t1: Traversal<T1>, k2: K2, l3: Lens<T1[K2], A3>): (f: (v: A3) => A3) => <S extends Collection<T1 & HasKey<K2>>>(s: S) => S"
  --       it "PPLL" do
  ...
  --       it "LLPTP" do
  --         (pprint $ (lens >>> lens >>> path >>> traversal >>> path) modbase) `shouldEqual` "export function mod<S1, A1, A2 extends HasKey<K3, Collection<T4>>, K3 extends string, T4, K5 extends string>(l1: Lens<S1, A1>, l2: Lens<A1, A2>, k3: K3, t4: Traversal<T4>, k5: K5): (f: (v: T4[K5]) => T4[K5]) => (s: S1) => S1"
  --       it "LLPTL" do
